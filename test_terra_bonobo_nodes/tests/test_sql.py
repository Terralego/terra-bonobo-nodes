...
class Test_TestSql_SQLExtract(unittest.TestCase):
    def test_sql_extractvalid(self):
        ...
        sql_query = """
            ...
            456::numeric AS B
        """
        sqlextract = sql.SQLExtract(
            db_alias=db_alias, identifier=identifier, sql_query=sql_query
        )
    ...
    def test_attributefromsql_valid(self):
        ...
        property_ = "property"
        record = {property_: "example_value"}

        sql_query = "SELECT 1::numeric"

        sqlextract = sql.AttributeFromSQL(
            db_alias=db_alias, property=property_, sql_query=sql_query
        )
