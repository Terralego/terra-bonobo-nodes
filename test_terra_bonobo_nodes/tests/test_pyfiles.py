...
class Test_TestPyfiles_PyfilesExtract(unittest.TestCase):
    ...
    def test_pyfilesextract_valid(self):
        with mock.patch.object(storages.core, "Storage") as mock_storage:
    ...
    def test_pyfilesextract_not_valid_response(self):
        with mock.patch.object(storages.core, "Storage") as mock_storage:
            ...
            with mock.patch.object(asyncio, "new_event_loop") as mock_event_loop:
    ...
    def test_pyfilesextract_not_valid_url(self):
        with mock.patch.object(storages.core, "Storage") as mock_storage:
            mock_storage.return_value.search.return_value = "whatever"
            with mock.patch.object(
                asyncio, "new_event_loop", return_value=mock.Mock(ok=True)
            ) as mock_event_loop:
                ...
                request = requests.Session()
                with mock.patch.object(
                    request, "get", return_value=mock.Mock(ok=False)
                ):
